...
const Navigation = () => {
    ...
    const navItems = ['Home', 'Features', 'Docs'];
    const navLinks = {
        'Home': '#',
        'Features': '#features',
        'Docs': '#'
    ...
    return (
        <>
            ...
            <header className="hidden sm:flex fixed top-0 left-0 right-0 z-50 justify-between items-center container mx-auto px-6 mt-6">
                    ...
                    <div className="flex items-center gap-1 bg-[#eff5fb]/60 border border-[#0b131e]/10 backdrop-blur-lg p-1.5 rounded-full shadow-lg">
                        {navItems.map(item => (
                            <a
                                key={item}
                                href={navLinks[item]}
                                className={`nav-item relative cursor-pointer text-sm font-semibold px-6 py-2 rounded-full transition-colors text-[#0b131e]/90 hover:text-[#2d74d7] ${activeItem === item ? 'active' : ''}`}
                                onClick={(e) => {
                                   if (navLinks[item] === '#') e.preventDefault();
                                   setActiveItem(item);
                                }}
                            >
                ...
                <button onClick={() => navigate('/login')} className="bg-[#2d74d7] text-[#eff5fb] font-semibold py-2 px-6 rounded-lg cta-button text-sm">Sign In</button>
            </header>

            {/* Mobile Navigation (original floating style) */}
            <nav id="tubelight-nav" className="sm:hidden fixed bottom-0 left-1/2 -translate-x-1/2 z-50 mb-6">
                <div className="flex items-center gap-1 bg-[#eff5fb]/60 border border-[#0b131e]/10 backdrop-blur-lg p-1.5 rounded-full shadow-lg">
                    {navItems.map(item => (
                        <a
                            key={item}
